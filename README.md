...
This application creates a workflow which runs on command after a REST API POST to API Gateway. It demonstrates the power of Step Functions to orchestrate Lambda functions and other AWS resources to form complex and robust workflows, coupled with event-driven development using Amazon EventBridge.
...
```bash
sam-step-functions-api-tutorial$ sam build
```
...
```bash
sam-step-functions-api-tutorial$ sam logs -n HelloWorldFunction --stack-name sam-step-functions-api-tutorial --tail
```
...
```bash
sam-step-functions-api-tutorial$ cd functions/hello-world
...
```bash
aws cloudformation delete-stack --stack-name sam-step-functions-api-tutorial
```
